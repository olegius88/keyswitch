...
AUTOCORRECTION_SETTINGS = (
    ...
    SettingSpec(
        ...
        "Дополнять набранный контекст текстом рядом с курсором через доступность ОС. Только локально; защищённые поля исключаются.",
        "bool",
    ),
    SettingSpec(
    ...
    SettingSpec(
        ...
        "Дополнительно проверять решение собственной символьной n-граммной моделью. Текст остаётся на компьютере.",
        "bool",
    ),
    SettingSpec(
    ...
    SettingSpec(
        ...
        "Раннее переключение по префиксу в обычном режиме. В режиме ИИ-помощника решение принимает контекстная модель после слова или паузы.",
        "bool",
    ),
    SettingSpec(
