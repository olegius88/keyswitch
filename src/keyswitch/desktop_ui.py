"""The KeySwitch settings window, in Tk, for every desktop that has one.

The window is the same on each platform: the same pages, the same settings, the
same wording. What differs - the keyboard backend, where autostart lives, how
programs are listed, how a folder opens, what the alert sound is - arrives as
:class:`~keyswitch.desktop_services.DesktopServices`, so nothing here knows
which system it is drawing on.
"""
...
class DesktopApplication:
    ...
    def __init__(
        ...
        *,
        ...
        self.engine = KeySwitchEngine(
            ...
            backend_label=services.backend_label,
        )
        self.autostart = services.autostart()
    ...
    def _sync_autostart(self, *, user_request: bool = False) -> None:
        ...
        try:
            self.autostart.set_enabled(
                ...
                # Only an explicit toggle overrides a choice the user made in the system.
                override_system_block=user_request,
            )
    ...
    def _sync_tray(self) -> None:
        ...
        if requested and self.tray is None:
            try:
                ...
                self.tray = self.services.tray(actions)
    ...
    def _open_log_directory(self) -> None:
        try:
            self.services.open_directory(log_directory())
        except (RuntimeError, OSError) as error:
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            LOGGER.exception("Ошибка остановки клавиатурного слоя")
        if self.tray is not None:
            self.tray.close()
            self.tray = None
