...
Полный поток клавиатуры не записывается. В обычном режиме в памяти находится
текущее слово, а в историю при включённой опции попадают пары вида `ghbdtn → привет`.
...
Скачайте `KeySwitch-Setup-0.14.1-x64.exe` со страницы
...
также входит переносимый архив `KeySwitch-0.14.1-windows-x64.zip`.
...
Скачайте `keyswitch_0.14.1_amd64.deb` со страницы
...
- Windows Setup EXE версии 0.14.1 пока не подписан сертификатом издателя.
