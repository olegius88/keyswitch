...
Download `KeySwitch-Setup-0.12.0-x64.exe` from the
...
portable `KeySwitch-0.12.0-windows-x64.zip` archive.
...
Download `keyswitch_0.12.0_amd64.deb` from the
[latest release](https://github.com/olegius88/keyswitch/releases/latest), then
install it with:

```bash
sudo apt install ./keyswitch_0.12.0_amd64.deb
```
...
package="dist/keyswitch_0.12.0_$(dpkg --print-architecture).deb"
...
- The Windows 0.12.0 Setup EXE is not yet signed with a publisher certificate.
