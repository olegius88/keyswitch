...
[0.31.2](https://github.com/olegius88/keyswitch/releases/tag/v0.31.2).
...
The expanded [context-v2 corpus](model/context_v2/README.md) exposed limitations
of both its baseline and candidate; the candidate failed its quality conditions
and **is not deployed**. Contextual weights have changed since 0.15.0; the
[context-v1 report](model/context_v1/report.json) identifies the current artifact.
[Context-v3](model/context_v3/README.md) remains experimental and is not installed.
...
Download `KeySwitch-Setup-0.31.2-x64.exe` from the
[published 0.31.2 release](https://github.com/olegius88/keyswitch/releases/tag/v0.31.2) and run
...
portable `KeySwitch-0.31.2-windows-x64.zip` archive.
...
Download `keyswitch_0.31.2_amd64.deb` from the
[published 0.31.2 release](https://github.com/olegius88/keyswitch/releases/tag/v0.31.2), then
install it with:

```bash
sudo apt install ./keyswitch_0.31.2_amd64.deb
```
...
- The published Windows 0.31.2 Setup EXE is not signed with a publisher certificate.
