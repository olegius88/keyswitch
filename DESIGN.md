...
Актуальная схема приложения: 0.16.1. Базовая KSLM v20 и контекстный помощник —
разные модели с разными проверками. [Карта документации](docs/README.md).
...
3. В поддерживающем синтетический ввод поле пользователь печатает `ghbdtn` в
   английской раскладке и нажимает пробел; защита первого слова после ручного
   выбора языка уже завершена, исключений и запретов для этого случая нет.
...
`InputBackend -> KeyEvent -> буфер клавиш/контекста -> LanguageDetector -> ContextPolicy -> CorrectionPlan -> InputBackend`
...
- Windows hook может удержать Enter/Tab и ввод во время замены. XRecord
  только наблюдает уже доставленные события. Решение модели и инъекция
  выполняются вне клавиатурного hook; полный поток событий не сохраняется.
- В памяти находятся физический токен, последняя граница и до 512 символов
  недавнего наблюдаемого контекста. Поле/окно, навигация, потеря событий и
  истечение 45 секунд ограничивают применимость контекста. Опциональный
  UIA/AT-SPI добавляет до 512 символов до курсора и 128 после него; по умолчанию
  чтение выключено. Снимок должен соответствовать слову, а перед заменой
  повторно проверяются поле, выделение и суффикс.
- Буфер хранит физические клавиши, поэтому русские буквы на клавишах
  `, . ; [ ]` не теряются как пунктуация. Boundary-v2 сравнивает целое слово
  с вариантами буквального хвоста после завершения токена или разрешённой
  паузы. Неоднозначный знак не обрезает незаконченный префикс. Неуверенная
  сегментация запрещает автоматическую замену, но не явный Pause.
- Precision-first распознавание сначала применяет жёсткие защиты и явно
  ...
  краткий контекст и базовую линейную модель намерения. В `assist` отдельная
  четырёхклассовая модель может изменить статистическое решение детектора,
  но не обойти пользовательские правила/исключения и проверки целостности.
- В базовом детекторе валидное исходное слово, совпадение сразу в двух раскладках, URL, путь,
...
- Ранняя смена раскладки не ждёт границы слова: после каждой буквы (начиная с
  настраиваемой длины, по умолчанию 4) префикс проверяется по отсортированному
  индексу слов частотного словаря и основ Hunspell. Раскладка переключается,
  только если у префикса нет ни одного продолжения и он не слово в исходном
  языке, а альтернативное прочтение начинает много частотных слов другого
  языка (либо одно доминирующее). Префикс переписывается при отпускании
  клавиши последней буквы (синтетическое нажатие ещё удерживаемой клавиши
  X-сервер игнорирует, и буква терялась бы); буквы, нажатые до отпускания,
  входят в переписываемый префикс, а буква, пришедшая в старой раскладке в
  течение 0,5 с после переключения, переписывается отдельно. Слово попадает в историю и в
  отмену как одна коррекция только на границе слова, где детектор всё ещё
  может вернуть его обратно. При загруженном контекстном помощнике,
  `context_aware=true` и режиме `assist` ранняя замена отключена.
- `Pause` преобразует то, что набрано после последней границы слова:
  незавершённое слово, зависящие от раскладки символы (кавычка вместо `@`)
  или последнее завершённое слово. Если после него уже был ввод (второй
  пробел, Backspace, навигация), `Pause` только переключает раскладку и
  защищает следующее слово как ручная смена.
- Таймер паузы просыпается точно к истечению настраиваемой задержки, а не по
  сетке в 0,5 с; нажатия без отпускания старше 3 с считаются потерянными и не
  блокируют коррекцию. Переход в ту раскладку, которую движок сам только что
  выбрал, замеченный в течение 1,5 с, не считается ручным; переход в любую
  другую раскладку — ручной, даже сразу после исправления.
- Enter в подсказке после ручного преобразования сразу подтверждает правило
  и не отправляется приложению. Без такого подтверждения правило по умолчанию
  активируется после двух одинаковых ручных преобразований; порог настраивается.
  Отмена автоматического исправления создаёт запрет для этой пары. Обычный
  набранный текст в обучающую базу не попадает.
- Базовый контекст последнего слова хранится в памяти по имени приложения,
  а новая наблюдаемая фраза привязана к активному окну; смена поля и другие
  события инвалидируют состояние. Это не чтение всей истории чата.
- В Windows перехваченный Enter/Tab проходит операцию «проверить/заменить
  слово → передать действие один раз»; при срыве операции действие отменяется.
  В X11 такая задержка недоступна, коррекция по этим клавишам отключена.
  Отправка событий не доказывает конечный текст: `text_verified=false`.
  [Матрица ввода](docs/input-maturity.md) описывает остальные проверки исполнителя.
- Следующие пункты про линейный слой относятся к **базовой KSLM**, а не к
  контекстной политике. Он сравнивает обе расшифровки по знаковым символьным 1–5-граммам и
  ...
  артефакта; допустимое исходное слово остаётся без замены базовым детектором.
- Поставляемая KSLM обучается offline через FTRL-Proximal, калибруется на
...
- Feature schema v5 строит вход линейного слоя только из raw token обеих
  ...
  проверка и runtime не расходятся; значение входит в закреплённую hashes
...
- Schema 13 загружает отдельный frozen
  ...
  закреплённый в config вес `3.0` применяется только к train-роли, а оценочные роли
...
## Проверка качества базового детектора

Эти gates не оценивают `ContextPolicy` или нативное исполнение замены.
Рабочий контекстный классификатор — `context-v1-953375a70173`:
[описание и доказательства](docs/context-assistant.md).
Эксперимент 0.16.0 расширил данные, но кандидат не прошёл условия качества и
не заменил рабочие веса: [результаты](model/context_v2/README.md).
...
Закреплённый hashes gate хранит метод, multiplicity correction, число сравнений, confidence,
...
split максимальный общий margin, закреплённый hashes и сохраняющий всю selection policy,
...
- Программа никогда не пишет полный журнал клавиатуры. История содержит только
  выполненные пары замен с метаданными и может быть отключена. Технический
  журнал — отдельная опция, записывающая и неисправленные слова; он может
  содержать личные данные. Парольные поля распознаются не во всех приложениях.
...
- Linux-приложение проверяет тот же выпуск и уведомляет пользователя, но не
  пытается обойти системную авторизацию для замены DEB. Само приложение DEB
  не ставит: обновление выполняет APT. Пакет ставит conffile'ами источник
  `/etc/apt/sources.list.d/keyswitch.sources` и открытый ключ
  `/etc/apt/keyrings/keyswitch-archive-keyring.asc`, которому доверяет только
  этот источник, поэтому новая версия приходит штатными средствами системы.
  Отключение или удаление источника переживает обновления пакета, а `purge`
  убирает оба файла.
- `tools/apt_repository.py` собирает и подписывает публикуемый репозиторий,
  а его проверка читает готовое дерево самим APT по `file://`. Suite, компонент,
  архитектура и адрес не дублируются в коде: и сборка, и проверка, и пакет
  берут их из `packaging/debian/keyswitch.sources`. Задание `apt-repository`
  публикует репозиторий на GitHub Pages до создания выпуска и сверяет адрес
  Pages с адресом в пакете; [устройство и ключ](docs/apt-repository.md).
...
- Обе сборки также включают точный `resources/models/context_policy_v1.json`
  и до компиляции запускают `verify_context_model.py` и `verify_context_v2.py`.
  Кандидат context-v2 остаётся исследовательским файлом вне runtime; его verifier
  требует сохранения frozen v1-baseline. CI отдельно повторяет контекстное
  обучение, корпус, engine replay и нативный AT-SPI E2E. Эти отдельные шаги
  не включены в `release_pipeline.py`; [точный состав проверок](docs/verification.md).
- Рабочая boundary-v2 и prefix-v1 проходят отдельные gates и побайтную сверку
  включённых артефактов. Boundary-v1 остаётся отклонённым экспериментом.
  GUI-проверки используют общий `tools/run-gui-test.sh` и тестовые настройки
  GTK/libadwaita без портала документов; службы и права основной ОС не меняются.
...
- `tools/verify-native-deb.sh` проверяет метаданные, архитектуру, формат ELF,
  разрешение динамических библиотек, версию исполняемого файла, bundled KSLM,
  а также регистрацию источника APT и ключа как conffiles и их побайтное
  совпадение с файлами в `packaging/`.
...
- `packaging/build-windows.ps1` компилирует Windows-приложение закреплённой
  ...
  закреплённую hashes матрицу quality gates и её совпадение с metadata KSLM. В
