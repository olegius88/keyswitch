...
- Feature schema v5 строит вход линейного слоя только из raw token обеих
  ...
  namespace `keyswitch:intent-v21:physical-signature`.
- Schema 13 загружает отдельный frozen
  `unknown-typo-development-v21.json`, построенный model-blind до обучения.
...
- Linux-приложение проверяет тот же выпуск и уведомляет пользователя, но не
  ...
  обновление Linux не реализовано. Приложение открывает страницу выпуска;
  скачанный DEB пользователь явно устанавливает через APT.
