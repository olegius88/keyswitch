...
from fixture_values.counts import (
    ...
    CHOOSE_THRESHOLD_LOW_SCORING_TRUE_ROWS,
    CHOOSE_THRESHOLD_NET_BENEFIT_FLOOR,
    CHOOSE_THRESHOLD_STRICT_NET_BENEFIT,
    EXPECTED_IDENTIFIER_DROPOUT_FAMILIES,
    FEATURE_MASS_CONTEXTS_LOOP_COUNT,
    FEATURE_MASS_COUNT_LIMIT,
    FEATURE_MASS_EXPANSION_COUNTS,
    FEATURE_MASS_GENEROUS_COUNT_LIMIT,
    FEATURE_MASS_REPEAT_COUNT,
    HISTORICAL_CURRICULUM_COPY_COUNTS,
    HISTORICAL_CURRICULUM_COVERAGE_SAMPLE_COUNT,
    HISTORICAL_CURRICULUM_EXPECTED_REDUCED_ROWS,
    HISTORICAL_CURRICULUM_PAIR_SIZE,
    HISTORICAL_CURRICULUM_RELABEL_SECOND_SOURCE_INDEX,
    HISTORICAL_CURRICULUM_TRIGGER_COVERAGE_SAMPLES,
    LEGACY_LOOKAHEAD_EXPECTED_INPUT_ANCHORS,
    LEGACY_LOOKAHEAD_EXPECTED_INPUT_SEEDS,
    LEGACY_LOOKAHEAD_EXPECTED_PLANNED_CONVERT,
    LEGACY_LOOKAHEAD_EXPECTED_PLANNED_KEEP,
    LEGACY_LOOKAHEAD_FAMILY_BUDGET,
    LEGACY_LOOKAHEAD_SEEDS_PER_FAMILY,
    LOOKAHEAD_PLANNED_VARIANTS_PER_SEED,
    NATURAL_LOOKAHEAD_FAMILY_BUDGET,
    NATURAL_LOOKAHEAD_SEEDS_PER_FAMILY,
    SELECT_ROWS_BUDGET,
    SELECT_ROWS_DISTINCT_FAMILY_ROWS,
...
from fixture_values.scores import (
    ...
    CHOOSE_THRESHOLD_CANDIDATES_WITH_CEILING,
    CHOOSE_THRESHOLD_CEILING_CANDIDATE,
    CHOOSE_THRESHOLD_HIGH_CANDIDATE,
    CHOOSE_THRESHOLD_HIGH_CONVERT_CONFIDENCE,
    CHOOSE_THRESHOLD_LOW_CANDIDATE,
    CHOOSE_THRESHOLD_LOW_PROBABILITY,
    CHOOSE_THRESHOLD_LOW_RECALL_FLOOR,
    CHOOSE_THRESHOLD_MID_CANDIDATE,
    CHOOSE_THRESHOLD_MINIMUM_RECALL_FLOOR,
    CHOOSE_THRESHOLD_NARROW_CANDIDATES,
    CHOOSE_THRESHOLD_NET_BENEFIT_TOLERANCE,
    CHOOSE_THRESHOLD_NO_CONVERSION_FALSE_SCORE,
    CHOOSE_THRESHOLD_NO_CONVERSION_TRUE_SCORE,
    CHOOSE_THRESHOLD_PARTIAL_RECALL,
    CHOOSE_THRESHOLD_PORTABLE_FALSE_SCORE,
    CHOOSE_THRESHOLD_REFERENCE_FALSE_SCORE,
    CHOOSE_THRESHOLD_VERY_LOW_FALSE_SCORE,
    EXPECTED_RECIPE_NET_BENEFIT_TOLERANCE,
    FEATURE_MASS_BASE_MASS,
    FEATURE_MASS_COMPARISON_DELTA,
    FEATURE_MASS_CUTOFF,
    FEATURE_MASS_EXCLUDED_MASS,
    FEATURE_MASS_FAMILY_A_MASS,
    FEATURE_MASS_INCLUDED_MASS,
    FEATURE_MASS_LOOSER_CUTOFF,
    FEATURE_MASS_NEAR_TIE_MASS,
    FEATURE_MASS_NEW_CONTEXT_MASS,
    FEATURE_MASS_OTHER_MASS,
    FEATURE_MASS_OVERFLOW_WEIGHT,
    FEATURE_MASS_SHARED_LOW_MASS,
    HISTORICAL_CURRICULUM_COMBINED_INDEPENDENT_MASS,
    HISTORICAL_CURRICULUM_CONVERT_MASS_SHARE,
    LEGACY_LOOKAHEAD_TOTAL_SEED_MASS,
    LOOKAHEAD_NEGATIVE_SEED_WEIGHT,
    LOOKAHEAD_POSITIVE_SEED_WEIGHT,
    PLANNED_BALANCE_CONTEXT_CONVERT_WEIGHT,
    PLANNED_BALANCE_FIRST_CONVERT_WEIGHT,
    PLANNED_BALANCE_ISOLATED_MASS_TOTAL,
    PLANNED_BALANCE_ISOLATED_SUGGEST_WEIGHT,
    PLANNED_BALANCE_KEEP_WEIGHT,
    PLANNED_BALANCE_SCALE_FACTOR,
    PLANNED_BALANCE_SECOND_CONVERT_WEIGHT,
)
...
class ActionTrainingTests(unittest.TestCase):
    ...
    def test_fractional_variants_preserve_vocabulary_ranking_and_context_mass(self) -> None:
        ...
        original.add({"family-a": FEATURE_MASS_FAMILY_A_MASS, "shared": FEATURE_MASS_SHARED_LOW_MASS}, 1.0)
        for _ in range(FEATURE_MASS_REPEAT_COUNT):
            original.add({"family-b": 1.0, "shared": 1.0}, 1.0)
        expected = select_features(original.values, FEATURE_MASS_CUTOFF, FEATURE_MASS_COUNT_LIMIT)
        self.assertEqual(expected, ["family-b", "shared"])
        for count in FEATURE_MASS_EXPANSION_COUNTS:
            expanded = FeatureMass()
            for _ in range(count):
                expanded.add({"family-a": FEATURE_MASS_FAMILY_A_MASS, "shared": FEATURE_MASS_SHARED_LOW_MASS}, 1.0 / count)
            for _ in range(FEATURE_MASS_REPEAT_COUNT):
                expanded.add({"family-b": 1.0, "shared": 1.0}, 1.0)
            self.assertEqual(select_features(expanded.values, FEATURE_MASS_CUTOFF, FEATURE_MASS_COUNT_LIMIT), expected)
            for name, value in original.values.items():
                self.assertAlmostEqual(expanded.values[name], value, delta=FEATURE_MASS_COMPARISON_DELTA)
        contexts = FeatureMass()
        for index in range(FEATURE_MASS_CONTEXTS_LOOP_COUNT):
            features = {"family-a": 1.0, "shared": 1.0}
            if index == 0:
                features["new-context"] = FEATURE_MASS_NEW_CONTEXT_MASS
            contexts.add(features, FEATURE_MASS_CUTOFF)
        for _ in range(FEATURE_MASS_REPEAT_COUNT):
            contexts.add({"family-b": 1.0, "shared": 1.0}, 1.0)
        self.assertEqual(contexts.values["new-context"], FEATURE_MASS_CUTOFF)
        self.assertEqual(select_features(contexts.values, FEATURE_MASS_CUTOFF, FEATURE_MASS_COUNT_LIMIT), expected)
        self.assertNotIn("new-context", select_features(contexts.values, FEATURE_MASS_LOOSER_CUTOFF, FEATURE_MASS_GENEROUS_COUNT_LIMIT))

    def test_feature_mass_cutoff_tolerance_and_rank_ties_are_deterministic(self) -> None:
        masses = {"under": FEATURE_MASS_EXCLUDED_MASS, "near": FEATURE_MASS_INCLUDED_MASS, "exact": FEATURE_MASS_BASE_MASS}
        self.assertEqual(select_features(masses, FEATURE_MASS_BASE_MASS, FEATURE_MASS_GENEROUS_COUNT_LIMIT), ["exact", "near"])
        tied = {"z": FEATURE_MASS_BASE_MASS, "a": FEATURE_MASS_NEAR_TIE_MASS, "other": FEATURE_MASS_OTHER_MASS}
        self.assertEqual(select_features(tied, FEATURE_MASS_BASE_MASS, 1), ["a"])
        self.assertEqual(select_features(dict(reversed(list(tied.items()))), FEATURE_MASS_BASE_MASS, 1), ["a"])
    ...
    def test_feature_mass_rejects_invalid_weights_limits_and_overflow(self) -> None:
        ...
        overflow.add({"name": 1.0}, FEATURE_MASS_OVERFLOW_WEIGHT)
    ...
    def test_historical_reduction_keeps_different_context_actions_and_application_roles(self) -> None:
        ...
        self.assertEqual(len(actual), HISTORICAL_CURRICULUM_EXPECTED_REDUCED_ROWS)
    ...
    def test_legacy_variants_share_the_original_family_category_trigger_budget(self) -> None:
        ...
        for source in (rows, expanded, expanded + expanded):
            ...
            self.assertAlmostEqual(sum(row.sample_weight for row in actual if row.action == "convert"), HISTORICAL_CURRICULUM_CONVERT_MASS_SHARE)
        independent = self.historical_fixture(expanded + [replace(rows[0], category="trusted_short_wrong")])
        self.assertAlmostEqual(sum(row.sample_weight for row in independent), HISTORICAL_CURRICULUM_COMBINED_INDEPENDENT_MASS)
    ...
    def test_calibration_uses_the_same_language_support_as_runtime(self) -> None:
        ...
        probabilities = array("d", [ACTION_TRAINING_RESIDUAL_PROBABILITY, ACTION_TRAINING_CONVERT_PROBABILITY, 0, 0, ACTION_TRAINING_RESIDUAL_PROBABILITY, ACTION_TRAINING_CONVERT_PROBABILITY, 0, 0])
        ...
        self.assertEqual(probabilities[ACTION_COUNT + 1], ACTION_TRAINING_CONVERT_PROBABILITY)
    ...
    def test_more_context_variants_do_not_dilute_another_legacy_action(self) -> None:
        ...
        for copies in HISTORICAL_CURRICULUM_COPY_COUNTS:
    ...
    def test_foreign_insertion_context_does_not_determine_layout_label(self) -> None:
        for original, group in (("deployment", 0), ("обсуждение", 1)):
            with self.subTest(group=group):
                ...
                self.assertEqual(len(pair), HISTORICAL_CURRICULUM_PAIR_SIZE)
                keep, wrong = pair
    ...
    def test_boundary_application_and_optional_context_vary_independently_of_trigger(self) -> None:
        rows = [row for number in range(HISTORICAL_CURRICULUM_COVERAGE_SAMPLE_COUNT)
    ...
    def test_selection_balances_languages_and_caps_repeated_families_before_scoring(self) -> None:
        rows = [fixture(str(index), "слово", 1, family="repeated") for index in range(SELECT_ROWS_REPEATED_FAMILY_ROWS)]
        rows += [fixture("en" + str(index), "word", 0) for index in range(SELECT_ROWS_DISTINCT_FAMILY_ROWS)]
        rows.append(replace(fixture("unsupported", "日本語", 0), layout_representable=False))
        rows.append(fixture("coarse-only", "can’t", 0))
        selected = select_rows(rows, SELECT_ROWS_BUDGET)
        self.assertEqual(sum(row.group == 1 for row in selected), MAX_CONTEXTS_PER_FAMILY)
        self.assertEqual(sum(row.group == 0 for row in selected), SELECT_ROWS_BUDGET // LAYOUT_GROUP_COUNT)
        self.assertEqual(selected, select_rows(list(reversed(rows)), SELECT_ROWS_BUDGET))

    def test_threshold_rejects_false_intervention_in_either_profile(self) -> None:
        threshold, report, passed = choose_threshold(
            {"portable": scores(CHOOSE_THRESHOLD_PORTABLE_FALSE_SCORE, CHOOSE_THRESHOLD_HIGH_CONVERT_CONFIDENCE),
             "reference_hunspell": scores(CHOOSE_THRESHOLD_REFERENCE_FALSE_SCORE, CHOOSE_THRESHOLD_HIGH_CONVERT_CONFIDENCE)},
            CHOOSE_THRESHOLD_CANDIDATES, 0, CHOOSE_THRESHOLD_MINIMUM_RECALL_FLOOR,
        )
    ...
    def test_serving_threshold_never_drops_below_the_development_operating_threshold(self) -> None:
        predictions = {"portable": scores(CHOOSE_THRESHOLD_PORTABLE_FALSE_SCORE, CHOOSE_THRESHOLD_HIGH_CONVERT_CONFIDENCE),
                       "reference_hunspell": scores(CHOOSE_THRESHOLD_PORTABLE_FALSE_SCORE, CHOOSE_THRESHOLD_HIGH_CONVERT_CONFIDENCE)}
        threshold, report, passed = choose_threshold(predictions, CHOOSE_THRESHOLD_CANDIDATES_WITH_CEILING, 0, CHOOSE_THRESHOLD_MINIMUM_RECALL_FLOOR)
        self.assertEqual((threshold, passed), (CHOOSE_THRESHOLD_MID_CANDIDATE, True))
        threshold, report, passed = choose_threshold(predictions, CHOOSE_THRESHOLD_CANDIDATES_WITH_CEILING, 0, CHOOSE_THRESHOLD_MINIMUM_RECALL_FLOOR, minimum_threshold=CHOOSE_THRESHOLD_CEILING_CANDIDATE)
        self.assertEqual((threshold, passed), (CHOOSE_THRESHOLD_CEILING_CANDIDATE, True))
        self.assertEqual(report["false_conversions"], 0)
        with self.assertRaises(ValueError):
            choose_threshold(predictions, CHOOSE_THRESHOLD_NARROW_CANDIDATES, 0, CHOOSE_THRESHOLD_MINIMUM_RECALL_FLOOR, minimum_threshold=CHOOSE_THRESHOLD_HIGH_CANDIDATE)
    ...
    def test_within_the_tolerance_the_safer_threshold_is_served(self) -> None:
        ...
        for index in range(CHOOSE_THRESHOLD_CONVERT_ROW_COUNT):
            probability = CHOOSE_THRESHOLD_HIGH_CANDIDATE if index < CHOOSE_THRESHOLD_CONVERT_ROW_COUNT - CHOOSE_THRESHOLD_LOW_SCORING_TRUE_ROWS else CHOOSE_THRESHOLD_LOW_PROBABILITY  # four correct conversions sit low
            values.extend([1 - probability, probability, 0.0, 0.0])
            labels.append(1)
        for index in range(CHOOSE_THRESHOLD_FALSE_ROW_COUNT):
            probability = CHOOSE_THRESHOLD_LOW_PROBABILITY if index < CHOOSE_THRESHOLD_LOW_SCORING_FALSE_ROWS else CHOOSE_THRESHOLD_HIGH_CANDIDATE  # three false conversions sit low too
            values.extend([1 - probability, probability, 0.0, 0.0])
            labels.append(0)
        predictions = {"portable": (values, labels)}
        greedy, greedy_report, _ = choose_threshold(predictions, CHOOSE_THRESHOLD_NARROW_CANDIDATES, 0, 0.0)
        self.assertEqual((greedy, greedy_report["net_benefit"], greedy_report["false_conversions"]), (CHOOSE_THRESHOLD_LOW_CANDIDATE, CHOOSE_THRESHOLD_GREEDY_NET_BENEFIT, CHOOSE_THRESHOLD_FALSE_ROW_COUNT))
        strict, report, passed = choose_threshold(predictions, CHOOSE_THRESHOLD_NARROW_CANDIDATES, 0, 0.0, net_benefit_tolerance=CHOOSE_THRESHOLD_NET_BENEFIT_TOLERANCE)
        self.assertEqual((strict, passed), (CHOOSE_THRESHOLD_MID_CANDIDATE, True))
        self.assertEqual((report["net_benefit"], report["false_conversions"]), (CHOOSE_THRESHOLD_STRICT_NET_BENEFIT, 1))
        self.assertEqual(report["admissible_thresholds"], CHOOSE_THRESHOLD_NARROW_CANDIDATES)
        self.assertEqual((report["net_benefit_ceiling"], report["net_benefit_floor"]), (CHOOSE_THRESHOLD_GREEDY_NET_BENEFIT, CHOOSE_THRESHOLD_NET_BENEFIT_FLOOR))
    ...
    def test_the_band_keeps_the_threshold_among_the_cases_the_product_promised(self) -> None:
        ...
        values, labels = array("d"), array("B")
        for index in range(CHOOSE_THRESHOLD_CONVERT_ROW_COUNT):
            probability = CHOOSE_THRESHOLD_HIGH_CANDIDATE if index < CHOOSE_THRESHOLD_CONVERT_ROW_COUNT - CHOOSE_THRESHOLD_LOW_SCORING_TRUE_ROWS else CHOOSE_THRESHOLD_LOW_PROBABILITY
            values.extend([1 - probability, probability, 0.0, 0.0])
            labels.append(1)
        for index in range(CHOOSE_THRESHOLD_FALSE_ROW_COUNT):
            probability = CHOOSE_THRESHOLD_LOW_PROBABILITY if index < CHOOSE_THRESHOLD_LOW_SCORING_FALSE_ROWS else CHOOSE_THRESHOLD_HIGH_CANDIDATE
            values.extend([1 - probability, probability, 0.0, 0.0])
            labels.append(0)
        predictions = {"portable": (values, labels)}
        capped, report, passed = choose_threshold(predictions, CHOOSE_THRESHOLD_NARROW_CANDIDATES, 0, 0.0,
                                                  net_benefit_tolerance=CHOOSE_THRESHOLD_NET_BENEFIT_TOLERANCE, maximum_threshold=CHOOSE_THRESHOLD_LOW_CANDIDATE)
        self.assertEqual((capped, passed), (CHOOSE_THRESHOLD_LOW_CANDIDATE, True))
        self.assertEqual(report["admissible_thresholds"], [CHOOSE_THRESHOLD_LOW_CANDIDATE])
        lifted, report, passed = choose_threshold(predictions, CHOOSE_THRESHOLD_NARROW_CANDIDATES, 0, 0.0,
                                                  net_benefit_tolerance=CHOOSE_THRESHOLD_NET_BENEFIT_TOLERANCE, authored_floor=CHOOSE_THRESHOLD_AUTHORED_FLOOR)
        self.assertEqual((lifted, passed), (CHOOSE_THRESHOLD_MID_CANDIDATE, True))
        self.assertEqual(report["admissible_thresholds"], [CHOOSE_THRESHOLD_MID_CANDIDATE])
        with self.assertRaises(ValueError):
            choose_threshold(predictions, CHOOSE_THRESHOLD_NARROW_CANDIDATES, 0, 0.0, minimum_threshold=CHOOSE_THRESHOLD_MID_CANDIDATE, maximum_threshold=CHOOSE_THRESHOLD_LOW_CANDIDATE)
        with self.assertRaises(ValueError):
            choose_threshold(predictions, CHOOSE_THRESHOLD_NARROW_CANDIDATES, 0, 0.0, authored_floor=CHOOSE_THRESHOLD_HIGH_CANDIDATE, maximum_threshold=CHOOSE_THRESHOLD_MID_CANDIDATE)
        recipe = json.loads((ROOT / "model/context_v3/recipe.json").read_bytes())
    ...
    def test_no_conversion_is_not_a_successful_calibration(self) -> None:
        _, report, passed = choose_threshold({"portable": scores(CHOOSE_THRESHOLD_NO_CONVERSION_FALSE_SCORE, CHOOSE_THRESHOLD_NO_CONVERSION_TRUE_SCORE)}, [1.0], 0, CHOOSE_THRESHOLD_MINIMUM_RECALL_FLOOR)
        self.assertFalse(passed)
        self.assertEqual(report["false_conversions"], 0)
        self.assertEqual(report["conversion_recall"], 0.0)
        self.assertEqual(metrics(array("d"), array("B"), CHOOSE_THRESHOLD_LOW_CANDIDATE)["conversion_recall"], 0.0)

    def test_a_good_profile_cannot_mask_a_failed_profile(self) -> None:
        _, report, passed = choose_threshold(
            {"portable": scores(CHOOSE_THRESHOLD_VERY_LOW_FALSE_SCORE, CHOOSE_THRESHOLD_HIGH_CONVERT_CONFIDENCE),
             "reference_hunspell": scores(CHOOSE_THRESHOLD_VERY_LOW_FALSE_SCORE, CHOOSE_THRESHOLD_PARTIAL_RECALL)},
            [CHOOSE_THRESHOLD_LOW_CANDIDATE], 0, CHOOSE_THRESHOLD_LOW_RECALL_FLOOR,
        )
        self.assertEqual(report["conversion_recall"], CHOOSE_THRESHOLD_PARTIAL_RECALL)
        self.assertFalse(passed)
        with self.assertRaises(ValueError):
            choose_threshold({}, [CHOOSE_THRESHOLD_LOW_CANDIDATE], 0, CHOOSE_THRESHOLD_MINIMUM_RECALL_FLOOR)
        with self.assertRaises(ValueError):
            choose_threshold({"portable": scores(CHOOSE_THRESHOLD_VERY_LOW_FALSE_SCORE, CHOOSE_THRESHOLD_MINIMUM_RECALL_FLOOR)}, [], 0, CHOOSE_THRESHOLD_MINIMUM_RECALL_FLOOR)
...
class LegacyLookaheadIntegrationTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.convert = ActionRow("legacy-train:convert", "r", 0, FieldContext("Telegram", "public-training", "", "привет дальше"),
                                 "space", "", "convert", "legacy_short_lookahead", " ", LOOKAHEAD_POSITIVE_SEED_WEIGHT, parent_family="family-a")
        self.keep = ActionRow("legacy-train:keep", "r", 0, FieldContext("Code", "public-training", "const value = ", ""),
                              "space", "", "keep", "legacy_technical", " ", LOOKAHEAD_NEGATIVE_SEED_WEIGHT, parent_family="family-a")
    ...
    def lookahead(self, rows: list[ActionRow]) -> tuple[list[ActionRow], dict[str, object]]:
        return legacy_lookahead_rows(rows, self.detector, None, profile="portable",
                                     maximum_families=LEGACY_LOOKAHEAD_FAMILY_BUDGET, seeds_per_family=LEGACY_LOOKAHEAD_SEEDS_PER_FAMILY)
    ...
    def test_short_legacy_seeds_gain_planned_variants_and_keep_their_mass(self) -> None:
        ...
        self.assertAlmostEqual(cast(float, report["input_mass"]), LEGACY_LOOKAHEAD_TOTAL_SEED_MASS)
        self.assertAlmostEqual(cast(float, report["output_mass"]), LEGACY_LOOKAHEAD_TOTAL_SEED_MASS)
        self.assertAlmostEqual(sum(row.sample_weight for row in result), LEGACY_LOOKAHEAD_TOTAL_SEED_MASS)
        ...
        self.assertEqual((counts["input_seeds"], counts["input_anchors"], counts["planned_convert"], counts["planned_keep"]),
                         (LEGACY_LOOKAHEAD_EXPECTED_INPUT_SEEDS, LEGACY_LOOKAHEAD_EXPECTED_INPUT_ANCHORS, LEGACY_LOOKAHEAD_EXPECTED_PLANNED_CONVERT, LEGACY_LOOKAHEAD_EXPECTED_PLANNED_KEEP))
...
class RuntimePolicyMaskTests(unittest.TestCase):
    ...
    def test_calibration_and_the_runtime_leave_the_verdict_to_the_model(self) -> None:
        ...
        convert = [ACTION_TRAINING_RESIDUAL_PROBABILITY, ACTION_TRAINING_CONVERT_PROBABILITY, 0.0, 0.0]
        ...
        keep = array("d", [ACTION_TRAINING_CONVERT_PROBABILITY, ACTION_TRAINING_RESIDUAL_PROBABILITY, 0.0, 0.0] * ACTION_COUNT)
    ...
    def test_identifier_evidence_dropout_is_deterministic_per_family_and_train_only(self) -> None:
        ...
        self.assertEqual(IDENTIFIER_DROPOUT_FAMILIES, EXPECTED_IDENTIFIER_DROPOUT_FAMILIES)
...
class NaturalLookaheadIntegrationTests(unittest.TestCase):
    ...
    def test_field_start_one_letter_word_gains_a_planned_variant_from_its_own_sentence(self) -> None:
        ...
        result, report = natural_lookahead_rows(rows, [source], self.detector, None, profile="portable", split="train",
                                                maximum_families=NATURAL_LOOKAHEAD_FAMILY_BUDGET, seeds_per_family=NATURAL_LOOKAHEAD_SEEDS_PER_FAMILY)
        planned = [row for row in result if row.after_origin == "planned_next_conversion"]
...
class PlannedMassBalanceTests(unittest.TestCase):
    ...
    def test_planned_frames_take_the_isolated_deferred_mass_of_their_class(self) -> None:
        ...
        rows = [
            ...
            ActionRow("ctx:1", "e", 0, FieldContext("Telegram", "public-training", "слово ", ""), "space", "", "convert", "layout_intervention", " ", PLANNED_BALANCE_CONTEXT_CONVERT_WEIGHT),
            ActionRow("planned:1", "e", 0, planned_field, "space", "", "convert", "legacy_short_lookahead", " ", PLANNED_BALANCE_FIRST_CONVERT_WEIGHT, after_origin="planned_next_conversion"),
            ActionRow("planned:2", "r", 0, planned_field, "space", "", "convert", "legacy_short_lookahead", " ", PLANNED_BALANCE_SECOND_CONVERT_WEIGHT, after_origin="planned_next_conversion"),
            ActionRow("planned:ru", "у", 1, planned_field, "space", "", "keep", "natural_surface", " ", PLANNED_BALANCE_KEEP_WEIGHT, after_origin="planned_next_conversion"),
            ActionRow("long", "hello", 0, alone, "space", "", "keep", "natural_surface", " ", 1.0),
        ]
        result, report = balance_planned_mass(rows)
        by_id = {row.identifier: row for row in result}
        self.assertAlmostEqual(by_id["planned:1"].sample_weight + by_id["planned:2"].sample_weight, PLANNED_BALANCE_ISOLATED_MASS_TOTAL)
        self.assertAlmostEqual(by_id["planned:1"].sample_weight / by_id["planned:2"].sample_weight,
                               PLANNED_BALANCE_FIRST_CONVERT_WEIGHT / PLANNED_BALANCE_SECOND_CONVERT_WEIGHT)
        self.assertEqual(by_id["planned:ru"].sample_weight, PLANNED_BALANCE_KEEP_WEIGHT)
        for name in ("iso:1", "iso:2", "iso:3", "ctx:1", "long"):
            self.assertEqual(by_id[name].sample_weight, {"iso:3": PLANNED_BALANCE_ISOLATED_SUGGEST_WEIGHT, "ctx:1": PLANNED_BALANCE_CONTEXT_CONVERT_WEIGHT}.get(name, 1.0))
        self.assertEqual(cast(dict[str, float], report["scale"]), {"0:1": PLANNED_BALANCE_SCALE_FACTOR})
        self.assertEqual(cast(dict[str, float], report["isolated_mass"]), {"0:1": PLANNED_BALANCE_ISOLATED_MASS_TOTAL})
        self.assertAlmostEqual(cast(float, report["output_mass"]),
                               cast(float, report["input_mass"]) - (PLANNED_BALANCE_FIRST_CONVERT_WEIGHT + PLANNED_BALANCE_SECOND_CONVERT_WEIGHT) + PLANNED_BALANCE_ISOLATED_MASS_TOTAL)
...
class PlannedEvidenceTests(unittest.TestCase):
    ...
    def test_planned_frame_evidence_uses_the_next_word_as_its_only_context(self) -> None:
        ...
        detector = LanguageDetector({
            0: LanguageModel("en_US", {"hello": FIXTURE_WORD_FREQUENCY, "kb": FIXTURE_WORD_FREQUENCY}, "fixture", enable_spellcheck=False),
            1: LanguageModel("ru_RU", {"этого": FIXTURE_WORD_FREQUENCY, "привет": FIXTURE_WORD_FREQUENCY, "ли": PLANNED_EVIDENCE_DOMINANT_WORD_FREQUENCY}, "fixture", enable_spellcheck=False),
        })
