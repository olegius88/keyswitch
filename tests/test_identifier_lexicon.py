...
from keyswitch.identifier_lexicon import RESOURCE_PATH, VERSION_HASH_CHARACTERS, IdentifierLexicon
...
PACKAGED_IDENTIFIER_COUNT = 14160
# Bytes of filler used to build an oversized-looking resource file.
FILLER_BYTE_COUNT = 32
# Patched onto MAX_RESOURCE_BYTES to force the size refusal deterministically.
TINY_RESOURCE_LIMIT_BYTES = 16
# schema_version the loader does not understand.
INVALID_SCHEMA_VERSION = 2
# "name" must be a str; this is a value of the wrong type.
INVALID_NAME_VALUE = 3
# Frequency fed to the fixture LanguageModel entries below.
WORD_FREQUENCY = 20
# Sample DetectionDecision.model_probability / model_threshold for the evidence tests.
...
class IdentifierLexiconTests(unittest.TestCase):
    ...
    def test_oversized_resources_are_refused_before_parsing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_bytes(b"[" + b" " * FILLER_BYTE_COUNT + b"]")
            with patch("keyswitch.identifier_lexicon.MAX_RESOURCE_BYTES", TINY_RESOURCE_LIMIT_BYTES), self.assertRaises(ValueError):
    ...
    def test_loader_rejects_malformed_resources_and_reports_missing_ones(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            broken_cases: list[object] = [
                {**valid, "schema_version": INVALID_SCHEMA_VERSION}, {**valid, "identifiers": ["def", "abc"]},
                {**valid, "identifiers": ["abc", "abc"]}, {**valid, "identifiers": ["Abc"]},
                {**valid, "identifiers": ["ab"]}, {**valid, "identifiers": []},
                {**valid, "name": INVALID_NAME_VALUE}, [],
            ]
    ...
    def test_evidence_flags_identifiers_in_either_reading_and_features_expose_them(self) -> None:
        en = LanguageModel("en_US", {"hello": WORD_FREQUENCY}, "test", enable_spellcheck=False)
        ru = LanguageModel("ru_RU", {"привет": WORD_FREQUENCY}, "test", enable_spellcheck=False)
        detector = LanguageDetector({0: en, 1: ru})
        field = FieldContext("Terminal", "1", "run ", "", "text")
        wrong = DetectionDecision(
            True, "терфыр", "nthash", 1, 0, 0, "test", SCORE, SCORE, MODEL_PROBABILITY, MODEL_THRESHOLD
        )
        ...
        right = DetectionDecision(
            False, "nthash", "nthash", 0, 0, 0, "test", SCORE, SCORE, MODEL_PROBABILITY, MODEL_THRESHOLD
        )
        ...
        word = DetectionDecision(
            False, "привет", "привет", 1, 1, 1, "test", SCORE, SCORE, MODEL_PROBABILITY, MODEL_THRESHOLD
        )
